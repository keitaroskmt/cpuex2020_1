...
(* M {key:string, value:((Id.t * Type.t) list * KNormal.t)} -> KNormal.t -> KNormal.t *)
let rec g env = function (* インライン展開ルーチン本体 (caml2html: inline_g) *)
  ...
  | LetRec({ name = (x, t); args = yts; body = e1 }, e2) -> (* 関数定義の場合 (caml2html: inline_letrec) *)
      let env = 
        if size e1 > !threshold then env 
        else 
          (
            if S.mem x (fv e1) then (* 再帰関数 *)
              (if size e1 > !rec_threshold then env else M.add x (yts, e1) env)
            else
              M.add x (yts, e1) env
          ) in
