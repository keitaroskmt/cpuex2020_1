...
const std::vector<std::string> add_inst{"add", "addi"};
...
const std::vector<std::string> jj_inst{"jalr", "jr", "jal", "j", "ret"};
...
int load_ops(FILE *fp)
{
    ...
    while (fgets(buf, 100, fp) != NULL)
    {
        ...
        if (valid_op)
        {
            // if (std::find(add_inst.begin(), add_inst.end(), new_op.opcode) != add_inst.end())
            // {
            //     new_op.slot_1 = true;
            //     new_op.slot_2 = true;
            // }
            ...
            if (std::find(jj_inst.begin(), jj_inst.end(), new_op.opcode) != jj_inst.end())
            {
                new_op.truejump = true;
            }
